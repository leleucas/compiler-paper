...
if __name__ == "__main__":
    ...
    N = 128
    shape = (N, 4)
    priors = tf.Variable(tf.ones(shape))
    tblr = tf.Variable(tf.ones(shape))

    sfunc = tblr2bboxes
    sargs = [priors, tblr]

    # correctness check
    #assert np.allclose(fast_tblr2bboxes(*sargs).numpy(), tblr2bboxes(*sargs).numpy())

    # performance check
    time_before = time.time()

    '''for _ in range(100):
        ret = fast_tblr2bxoxes(*sargs)'''
